...
public class factors {
  public class factors {
    public static void main(String[] args) {
      ...
      for (int i = 1; i <= no1; i++) {
        if (no % i == 0) { // Check divisibility of the original number
          store.add(i);    // Add the divisor
          if (no / i != i) { // Avoid adding the square root twice
            store.add(no / i); // Add the corresponding pair divisor
          }
        }
      }
